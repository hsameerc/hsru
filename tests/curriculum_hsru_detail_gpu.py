...
# from core.hsru import HSRnn #CPU
from src.core.hsru_parallal import HSRnn #GPU
...
def plot_hidden_states(model, data_args, title, lr):
    ...
    plt.savefig(f"../plots/curriculum_hsru_detail_gpu/{title}/{title}-{data_args['seq_len']}-{str(lr)}.png")
    plt.close()
...
def run_final_benchmark_with_sweep():
    ...
    plt.savefig(f"../plots/curriculum_hsru_detail_gpu/HSRnn vs. LSTM on Final Curriculum Stage ({curriculum[-1]['title']}).png")
    plt.close()
